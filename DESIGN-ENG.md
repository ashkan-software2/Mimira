...
4. **Pre-send quota hard-block** for aftercare + broadcast (vs visibility-only as the plan currently has it — see [DESIGN.md](DESIGN.md) §Line API Constraints). Note: the design appendix's Unresolved #8 covers placement of the readout, but does not address enforcement.
