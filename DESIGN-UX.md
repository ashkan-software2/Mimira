...
| Page navigation — TopLoader bar | Thin accent bar appears at top instantly on link click and trickles forward while the next route renders (no full-page skeleton) | n/a (only shown during a navigation) | If navigation never completes, a 12 s safety timer completes and fades the bar so it never sticks; user can retry the link | Bar snaps to 100% and fades the moment the new route renders | Ignored cases (no bar): external links, new-tab/modified clicks, downloads, `#`-anchors, and same-page navigation |
